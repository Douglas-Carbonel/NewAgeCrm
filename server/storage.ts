...
export class MemStorage implements IStorage {
  ...
  async createContract(insertContract: InsertContract): Promise<Contract> {
    const contract: Contract = {
      ...
      clientId: insertContract.clientId,
  ...
  async createInvoice(insertInvoice: InsertInvoice): Promise<Invoice> {
    const invoice: Invoice = {
      ...
      isAutoGenerated: insertInvoice.isAutoGenerated ?? false,
      timeEntriesIds: insertInvoice.timeEntriesIds ?? [],
      createdAt: new Date(),
    };
...
import { isSupabaseConnected } from './db';

// Use Supabase storage if connected, otherwise fall back to in-memory storage
export const storage = isSupabaseConnected ? supabaseStorage : new MemStorage();
